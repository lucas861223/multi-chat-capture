...
for (index in channelList) {
  fetchChannelBadgesAndEmotes(channelList[index]);
}
  ...
  if (!self && message[0] === "!" && '#' + userstate["username"] === mainChannel && channel === mainChannel) {
    ...
    if (command == "join") {
      ...
      if (command === "size") {
        ...
        document.documentElement.style.setProperty('--background', parts[1]);
      }
    }
  } else {
    messageCounter++;
...
function formatMessage(message, emotes, channel) {
  ...
  for (emote in emotes) {
    ...
    matchedWords[message.substring(parseInt(emoteRange[0]), parseInt(emoteRange[1]) + 1)] = img + emote + '/2.0" /><span style:"font-size:12"> </span>';
  }
  ...
  for (token in tokens) {
    if (tokens[token] in matchedWords) {
      ...
      messageHTML += bttvImg + globalBTTVEmotes[tokens[token]] + '/2x" /><span style:"font-size:12"> </span>';
    } else if (tokens[token] in channelBTTVEmotes[channel]) {
      messageHTML += bttvImg + channelBTTVEmotes[channel][tokens[token]] + '/2x" /><span style:"font-size:12"> </span>';
    } else {
...
async function fetchChannelBadgesAndEmotes(channelName) {
  ...
  console.log(tmpJson, channelBTTVEmotes)
}
