...
client.on('message', async function (channel, userstate, message, self) {
  ...
  if (!self && message[0] === "!" && '#' + userstate["username"] === mainChannel && channel === mainChannel) {
    ...
    if (command == "join") {
      ...
      for (i in leavingChannelList) {
        ...
        if (index > -1 && '#' + leavingChannelList[i] != mainChannel) {
          ...
          delete channelFFZEmotes['#' + leavingChannelList[i]];
    ...
    highLighted = !mentionHighlightOvverride && message.toLowerCase().includes('@' + mainChannel.toLowerCase().slice(1));
    if (highLighted) {
    ...
    if (userstate['message-type'] === 'action') {
      isAction = true;
      if (!highLighted) {
        formattedMessage = '<span style="color:' + (userstate['color'] ? userstate['color'] : userColor[userstate['username']]) + '">' + formattedMessage + '</span>';
      }
    }
    chatContainer.insertAdjacentHTML('beforeEnd', containerHTML + badges + username + (isAction ? ' ' : '<b>: </b>') + formattedMessage + '</div>');
...
function formatMessage(message, emotes, channel) {
  ...
  ffzImg = '<img class="emote" src="https:';
  var emoteRange;
  for (emote in emotes) {
  ...
  inSpan = false;
  for (token in tokens) {
    if (tokens[token].toLowerCase() === "every" || tokens[token].toLowerCase() === "some") {
      if (!inSpan) {
        inSpan = true;
        messageHTML += "<span>"
      }
      messageHTML += tokens[token] + ' ';
    } else if (matchedWords[tokens[token]]) {
      isEmote = true;
      messageHTML += matchedWords[tokens[token]];
    } else if (globalBTTVEmotes[tokens[token]]) {
      if (inSpan) {
        inSpan = false;
        messageHTML += "</span>"
      }
      messageHTML += bttvImg + globalBTTVEmotes[tokens[token]] + '/2x" /> ';
    } else if (channelBTTVEmotes[channel][tokens[token]]) {
      if (inSpan) {
        inSpan = false;
        messageHTML += "</span>"
      }
      messageHTML += bttvImg + channelBTTVEmotes[channel][tokens[token]] + '/2x" /> ';
    } else if (channelFFZEmotes[channel][tokens[token]]) {
      if (inSpan) {
        inSpan = false;
        messageHTML += "</span>"
      }
      messageHTML += ffzImg + channelFFZEmotes[channel][tokens[token]] + '" /> ';
    } else if (globalFFZEmotes[tokens[token]]) {
      if (inSpan) {
        inSpan = false;
        messageHTML += "</span>"
      }
      messageHTML += ffzImg + globalFFZEmotes[tokens[token]] + '" /> ';
    } else {
      if (!inSpan) {
        inSpan = true;
        messageHTML += "<span>"
...
async function fetchChannelBadgesAndEmotes(channelName) {
  ...
  tmpJson = await fetch('https://api.frankerfacez.com/v1/room/' + channelName).then((res) => res.json());
  channelFFZEmotes[channel] = {};
  if ('sets' in tmpJson) {
    for (set in tmpJson['sets']) {
      for(i = 0; i < tmpJson['sets'][set]['emoticons'].length; i++) {
        channelFFZEmotes[channel][tmpJson['sets'][set]['emoticons'][i].name] = tmpJson['sets'][set]['emoticons'][i].urls['1'];
      }
    }
  }
}
...
async function prepareGlobalEmotes() {
  ...
  tmpJson = await fetch('https://api.frankerfacez.com/v1/set/global').then((res) => res.json());
  for (i = 0; i < tmpJson['default_sets'].length; i++) {
    for (var emote in tmpJson['sets'][tmpJson['default_sets'][i]]['emoticons']) {
      globalFFZEmotes[tmpJson['sets'][tmpJson['default_sets'][i]]['emoticons'][emote].name] = tmpJson['sets'][tmpJson['default_sets'][i]]['emoticons'][emote]['urls']['1'];
    }
  }
}